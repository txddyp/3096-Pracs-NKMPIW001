...
void EXTI0_1_IRQHandler(void)
{
	...
	//int tick = 100;
	//int tStart = HAL_GetTick();

	//if ((HAL_GetTick()-tickStart) <= tick)
	//{
		//HAL_GPIO_TogglePin(GPIOx, GPIO_Pin_8); //to toggle the blue led
		//HAL_Delay(500); //equivalent to 2 Hz
		//HAL_GPIO_EXTI_IRQHandler(B1_Pin); // Clear interrupt flags
	//}
	//HAL_GPIO_TogglePin(GPIOx, GPIO_Pin_8);
	if (delay == 500)
	{
		delay = 1000;
	}
...
uint32_t pollADC(void){
	...
	int adcVal = HAL_ADC_GetValue(&hadc);
	HAL_ADC_Stop(&hadc);
	HAL_Delay(delay);
	//sprintf(buffer, "adc = %d", adcVal);
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	uint32_t crrValue = (adc_val/4096.0)*47999;
