...
void pause_sec(float x)
{
	...
	for (int j = 0; j<((int)x*DELAY1);j++){
		for (int i;i<DELAY2;i++){
...
int bcdToDec(uint8_t val)
{
	...
	//YOUR CODE HERE
	return (int) (((val/16)*10)+(val%16));  //&0x0F));
...
int epochFromTime(TIME time){
	...
	//YOUR CODE HERE
	int daySecs = 86400; //seconds in a day
	int result = 0;
	int days = 0;

	int months[13] = {0,31,28,31,30,31,30,31,31,30,31,30,31};
	result = result + ((time.year-22)*daySecs*365);

	for (int i =1;i<time.month;i++){  //loops through to find the amount of complete months that have passed
		days += months[i];
	}

	days += (time.dayofmonth -1); //Adds the number of complete days that have passed
	result += (days*daySecs);  //Adds the days in seconds to the total amount
	result += (time.hour*3600); //Adds the amount of hours in seconds elapsed to the total amount
	result += (time.minutes*60);//Adds the amount of minutes in seconds elapsed to the total amount
	result += (time.seconds);//Adds the amount of seconds elapsed to the total amount
